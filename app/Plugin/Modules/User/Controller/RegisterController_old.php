...
class RegisterController extends UserAppController {
    ...
    public function index() {
        ...
        if ($this->request->isPost()) {
            ...
            $relatedForms = array('PatientDisease');
            $validationGroups = array();
            $this->JQValidator->addValidation('User', $validationOptions, $formId, $validationGroups, $relatedForms);
            ...
            $this->set(compact('countryList', 'formId', 'dob', 'patientRelation', 'defaultDOBStartingYear'));
        }
    }
        ...
        if ($this->User->createUser($userDetails)) {
            ...
            switch ($userDetails['type']) {
                ...
                case User::ROLE_CAREGIVER:
                    $this->_addCareGiverPatient($id, $userData);
                    $this->_addPatientDiseases($id);
                    break;
				default:
					$this->_addPatientDiseases($id);
    ...
    /**
     * Add patient details of care giver
     * 
     * @param int $careGiverId
     * @param array $userData
     */
    private function _addCareGiverPatient($careGiverId, $userData) {
        $patientDOB = join('-', array(
            $userData['patient-dob-year'],
            $userData['patient-dob-month'],
            $userData['patient-dob-day'],
        ));
        $patientDetails = array(
            'relationship' => $userData['patient-relationship'],
            'care_giver_id' => $careGiverId,
            'first_name' => $userData['patient-firstname'],
            'last_name' => $userData['patient-lastname'],
            'date_of_birth' => $patientDOB,
            'gender' => $userData['patient-gender'],
            'country' => $userData['patient-country'],
            'state' => $userData['patient-state'],
            'city' => $userData['patient-city'],
            'zip' => $userData['patient-zip']
        );
        $this->CareGiverPatient->save($patientDetails);
    }
    ...
    public function registerRediredt($username = null) {
        ...
        $this->Session->setFlash(__('Congrats for signing up to Patients4Life. Please check your inbox for the activation link which will expire in 24 hours.'), 'success');
        if ($username != null) {
