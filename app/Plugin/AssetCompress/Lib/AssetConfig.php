 ...
 */
	public function __construct(array $data = array(), array $additionalConstants = array(), $modifiedTime = null) {
		$this->_data = $data;
		$additionalConstants['APP_THEME_WEBROOT/'] = App::themePath('App') . 'webroot/';
		$additionalConstants['ADMIN_THEME_WEBROOT/'] = App::themePath('Admin') . 'webroot/';
...
/**
 * Get the theme name of build target
 *
 * @param string $target A build target.
 * @return string
 */
	public function getThemeName($target) {
		$ext = $this->getExt($target);
		return $this->_data[$ext][self::TARGETS][$target]['theme_name'];
	}
