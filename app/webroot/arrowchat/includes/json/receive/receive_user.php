	...
	if (!empty($fetchid)) 
	{
		...
		if ($result AND $db->count_select() > 0) 
		{
			...
			$avatar = get_avatar($chat['avatar'], $fetchid);
                        $avatar_class = get_avatar_class($fetchid);

			$response =  array('id' => $chat['userid'], 'n' => $db->escape_string(strip_tags($chat['username'])), 's' => $chat['status'], 'a' => $avatar,'ac' => $avatar_class, 'l' => $link);
