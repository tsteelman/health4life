		...
		while ($chat = $db->fetch_array($result)) 
		{
			if ($chat['userid'] != $userid) 
			{
				...
				$avatar = get_avatar($chat['avatar'], $chat['userid']);
                                $avatar_class = get_avatar_class($chat['userid']);
				...
				if (!in_array($chat['userid'], $block_chats_unserialized))
				{
					...
					{
						$buddyList[] = array('id' => $chat['userid'], 'n' => stripslashes($db->escape_string(strip_tags($chat['username']))), 's' => $chat['status'], 'a' => $avatar,'ac' => $avatar_class, 'l' => $link, 'admin' => $chat['is_admin']);
					}
				}
			}
		}
