<?php
	...
	*/
	...
	function get_friend_list($userid, $time)
	{
		...
		$friendObj = new MyFriends();
		$friends = $friendObj->getFriendsList($userid);
		$uids = array();
		foreach($friends as $friend)
		{
		   array_push($uids, $friend['friend_id']);
		}   
		...
		$sqlQuery = "SELECT DISTINCT " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " userid, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " username, arrowchat_status.session_time lastactivity, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_AVATAR . " avatar, " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " link, arrowchat_status.is_admin, arrowchat_status.status
			...
			WHERE " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_USERID . " IN(" . $db->escape_string($friendsId) . ")"
			." AND (arrowchat_status.session_time + 60 + " . $online_timeout . ") > ".  time();
                
		$sqlQuery .= " ORDER BY " . TABLE_PREFIX . DB_USERTABLE . "." . DB_USERTABLE_NAME . " ASC";
	...
	{		
                $userObj = new User();
                $user = $userObj->getUserDetails($user_id);
                $image = Common::getUserThumb($user_id, $user['type'], 'x_small', 'media-object','');		
		
		return $image;
	}

        /**
	 * This function returns the avatar class of the specified user ID.
	 *
	 * @param userid the user ID of the user	
	 * @return the class of the user ID's profile
	 */
        function get_avatar_class($user_id)
        {
            $userObj = new User();
            $user = $userObj->getUserDetails($user_id);
            $border = Common::getUserThumbClass($user['type']);
            return $border;
        }
