...
class phpVimeo {
    ...
    public function __construct($config) {
        if (!empty($config) && is_array($config)) {
            ...
            if (isset($config['ACCESS_TOKEN']) && isset($config['ACCESS_TOKEN_SECRET'])) {
                $this->setToken($config['ACCESS_TOKEN'], $config['ACCESS_TOKEN_SECRET']);
            }
        }
    }
    ...
    /**
     * Get the thumbnail url of a vimeo video
     * 
     * @param string $video_id
     * @return string
     */
    public function getThumbnailUrl($video_id) {
        $params = array('video_id' => $video_id);
        $video = $this->call('vimeo.videos.getThumbnailUrls', $params);
        try {
            if (is_object($video) && !empty($video->thumbnails))
                return $video->thumbnails->thumbnail[2]->_content;
            else
                return null;
    ...
    public function getVideoInfo($video_id) {
        $params = array('video_id' => $video_id);
        $result = $this->call('vimeo.videos.getInfo', $params);
        try {
            if (!is_null($result) && is_object($result) && $result->stat === 'ok') {
    ...
    public function deleteVideo($video_id) {
        $params = array('video_id' => $video_id);
        $result = $this->call('vimeo.videos.delete', $params);
        try {
            if (!is_null($result) && is_object($result) && $result->stat === 'ok') {
                return true;
            } else {
