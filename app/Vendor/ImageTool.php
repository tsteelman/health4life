...
class ImageTool {
	...
	public static function resize($options = array()) {
		...
		$dst_im = self::imagecreatetransparent($options['width'], $options['height']);
	...
	* @return mixed boolean or GD resource if output was set to null
	*/
	public static function crop($options = array()) {
		...
		$targ_w = $options['output_width'];
		$targ_h = $options['output_height'];
		$src_x = $options['left'];
		$src_y = $options['top'];
		$src = $options['input'];		
		...
		$dst_im = self::imagecreatetransparent($targ_w, $targ_h);

		$r = imagecopyresampled($dst_im, $src_im, 0, 0, $src_x, $src_y, $targ_w, $targ_h, $options['width'], $options['height']);
		
		/* header('Content-type: image/jpeg');
		imagejpeg($dst_im,null,'90');
		exit; */
	...
	* @return  resource
	*/
	protected static function imagecreatetransparent($width, $height)
	{
	...
	/**
	 * Function to apply gaussian blur on the bottom of an image
	 * 
	 * @param string $imgPath
	 * @param int $height
	 */
	public static function gaussianBlurBottom($imgPath, $height = null) {
		$imageType = self::getImageType($imgPath);
		switch ($imageType) {
			case 'jpg':
		...
		// We only want to blur bottom of the image 
		// so copy bottom to a temporary image
		list($imageWidth, $imageHeight) = getimagesize($imgPath);
		if (is_null($height)) {
			$tmpImgHeight = floor($imageHeight / 4);
		} else {
			$tmpImgHeight = $height;
		}
		$tmpImgTop = $imageHeight - $tmpImgHeight;
		$tempImage = imagecreatetruecolor($imageWidth, $tmpImgHeight);
		imagecopy($tempImage, $image, 0, 0, 0, $tmpImgTop, $imageWidth, $imageHeight);

		// apply gaussian filter 100 times, to give blur effect
		for ($i = 0; $i < 100; ++$i) {
			imagefilter($tempImage, IMG_FILTER_GAUSSIAN_BLUR);
		}

		// copy it back to the original image.
		imagecopy($image, $tempImage, 0, $tmpImgTop, 0, 0, $imageWidth, $tmpImgHeight);
		imagedestroy($tempImage);
		
		// save the image as {filepath}_blur.extension
		switch ($imageType) {
			case 'jpg':
				$blurImgPath = str_ireplace('.jpg', '_blur.jpg', $imgPath);
				imagejpeg($image, $blurImgPath);
				break;
			case 'png':
				$blurImgPath = str_ireplace('.png', '_blur.png', $imgPath);
				imagepng($image, $blurImgPath);
				break;
			case 'gif':
				$blurImgPath = str_ireplace('.gif', '_blur.gif', $imgPath);
				imagegif($image, $blurImgPath);
				break;
		}
	}
}