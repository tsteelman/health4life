...
$.fn.extend({
	facelist: function(urlOrData, options) {
...
$.Autocompleter = function(input, options) {
    ...
    var $input_facelist=$(input).closest('.facelist');
    var $result_list=$input_facelist.next('.result_list');
	var facelistXhr;
	...
	$input_facelist.click(function (){ $input.focus();});
        $input.focusout(function (){
            if ($('.token').length === 0)
            { 
                $(input).attr('placeholder',$(input).data('placeholder')); 
            }            
            $('.default').remove();});//to remove the default when focus out
	var timeout;
	var previousValue = "";
	...
	$input.keydown(function(event) {
		...
		switch(event.keyCode) {
			...
			case KEY.DEL:
				...
				if($input.val() == "" && $input_facelist.find('#bit-'+data_tag) && $input_facelist.find('#bit-'+data_tag).attr('class') == "token token_selected")
				{
					Remove_tag_data(arr_user[arr_user.length - 2]);
				}
				else if($input.val() == "" && $input_facelist.find('#bit-'+arr_user[arr_user.length - 2]) && $input_facelist.find('#bit-'+arr_user[arr_user.length - 2]).attr('class') != "token token_selected")
				{
                                        $('.token').removeClass('token_selected'); 
                                      $input_facelist.find('#bit-'+data_tag).addClass('token_selected');
				}
				else if($input.val().length == 1)
				{
					$result_list.find('ul').remove();
					$result_list.find('div').remove();
                                        if ($(input).attr('placeholder') !== '')
                                        { $(input).attr('placeholder',$(input).data('placeholder')); }
					$result_list.append('<div class="default">'+options.intro_text+'</div>');
				}
	...
	function Remove_tag_data(data)
	{
		...
		$input_facelist.find('#bit-'+new String(data).replace(/ /g,'_')).remove();
	...
	function MakeBox(data)
        {
            $('.token').removeClass('token_selected');
            elemLI = $('<li id="bit-' + new String(data[1]).replace(/ /g, '_') + '" class="token"><span><span><span><span>' + data[0] + '</span></span></span></span></li>').click(function() {
                $('.token').removeClass('token_selected');
            ...
            $(elemLI).append(elemA);
            $result_list.find('ul').remove();
            $input.parent('.token-input').before(elemLI);
            $input.val('');            
            if ($(input).attr('placeholder') !== '')
            {
                $(input).data('placeholder', $(input).attr('placeholder'));
	...
	function selectCurrent() {
		...
		if($result_list.find('ul').get() != "" && $result_list.find('ul#no_result').get() == "")
		{	
	...
	function repairValueList(){
		...
		$('#'+options.result_field).trigger('change');
        
        // remove error message if input has value
        if (str !== "") {
            $(input).closest('.form-group-col').find('span.help-block').remove();
        }
	};
			...
			facelistXhr = $.get(
				...
				function(data)
				{
					...
					if(data.length == 0){
                                            ...
                                            /*
                                             * Custom code added for removing value from input field
                                             */
                                            $('#'+options.result_field).prev().val('');
					}
				}
				);		
		}
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	var listItems,
		...
		data,
		term = "",
		element = $(input).closest('.facelist').next('.result_list'),
		list;
	...
	function fillList() {
		...
		if(results == false){
			element.html('<div class="default">No Results Found</div>');
		}
