...
<!--                        <p>supporting</p>
					...
					<div class="team_members_details">
		               <span class="memebr_patient">
						   <?php if (is_null($team['TeamMember']['status'])): //not joined ?> 
								<button data-team_id="<?php echo $team['Team']['id']; ?>" class=" btn btn_normal public_join_team" type="button" >Join Team </button>							   
						  <?php else: //awaiting approval?>
								<a href="/myteam/<?php echo $team['Team']['id']; ?>">
								<button data-team_id="<?php echo $team['Team']['id']; ?>" class=" btn btn_normal" type="button" > Awaiting approval </button>
								</a>
						  <?php endif; ?>
						</span>
					</div>
